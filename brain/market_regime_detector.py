...
class AdvancedMarketRegimeDetector:
    ...
    async def _extract_regime_features(self, market_data: Dict) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            features['higher_highs_count'] = self._count_higher_highs(price_history[-20:])
            features['lower_lows_count'] = self._count_lower_lows(price_history[-20:])
            
            # Support/Resistance Strength
            features['support_strength'] = self._calculate_support_strength(price_history)
            features['resistance_strength'] = self._calculate_resistance_strength(price_history)
            
            # 6. Regime-Specific Features
            features['trend_consistency'] = self._calculate_trend_consistency(price_history)
            features['volatility_clustering'] = self._calculate_volatility_clustering(returns)
            features['return_distribution_skew'] = self._calculate_skewness(returns)
            features['return_distribution_kurtosis'] = self._calculate_kurtosis(returns)
            
            # 7. Cross-Asset Features (Mock)
            features['market_correlation'] = np.random.uniform(-0.3, 0.8)  # Mock correlation with broader market
            features['sector_rotation'] = np.random.uniform(-0.5, 0.5)    # Mock sector rotation indicator
    ...
    def _calculate_support_strength(self, prices: List[float]) -> float:
        """Support Level Strength"""
        if len(prices) < 10:
            return 0.5
        
        min_price = min(prices[-10:])
        touches = sum([1 for p in prices[-10:] if abs(p - min_price) / min_price < 0.02])
        return min(1.0, touches / 3.0)  # Normalize
    
    def _calculate_resistance_strength(self, prices: List[float]) -> float:
        """Resistance Level Strength"""
        if len(prices) < 10:
            return 0.5
        
        max_price = max(prices[-10:])
        touches = sum([1 for p in prices[-10:] if abs(p - max_price) / max_price < 0.02])
        return min(1.0, touches / 3.0)  # Normalize
    ...
    def _calculate_trend_consistency(self, prices: List[float]) -> float:
        ...
        positive_returns = sum([1 for r in returns[-9:] if r > 0])  # Last 9 returns
        consistency = positive_returns / 9.0 if len(returns) >= 9 else 0.5
        
        # Adjust for magnitude
        avg_return_magnitude = np.mean(np.abs(returns[-9:]))
        consistency_score = consistency * (1 + avg_return_magnitude * 10)  # Boost for strong trends
    ...
    def _calculate_volatility_clustering(self, returns: List[float]) -> float:
        ...
        return (autocorr + 1) / 2 if not np.isnan(autocorr) else 0.5  # Normalize to 0-1
    ...
    def _calculate_skewness(self, returns: List[float]) -> float:
        ...
        skewness = np.mean(((returns_array - mean_ret) / std_ret) ** 3)
    ...
    def _calculate_kurtosis(self, returns: List[float]) -> float:
        ...
        mean_ret = np.mean(returns_array)
        std_ret = np.std(returns_array)
        kurtosis = np.mean(((returns_array - mean_ret) / std_ret) ** 4)
        
        return np.clip(kurtosis, 1, 10)  # Clip extreme values 
    ...
    async def _train_models(self):
        ...
        try:
            ...
            logger.info(f"🎯 Feature Dimensions: {feature_matrix.shape[1]} → {reduced_features.shape[1]}")
    ...
    async def _advanced_regime_detection(self, features: Dict[str, float]) -> RegimeAnalysis:
        ...
        try:
            ...
            gmm_cluster = self.gmm_model.predict(reduced_features)[0]
            gmm_probabilities = self.gmm_model.predict_proba(reduced_features)[0]
    ...
    async def _fallback_regime_detection(self, features: Dict[str, float]) -> RegimeAnalysis:
        ...
        try:
            ...
            # Bull Trending
            if momentum_5 > 0.02 and momentum_20 > 0.05 and trend_consistency > 0.7:
            ...
            # Bear Trending  
            if momentum_5 < -0.02 and momentum_20 < -0.05 and trend_consistency < 0.3:
            ...
            # Bull Correction
            if momentum_20 > 0.02 and momentum_5 < -0.01 and bb_position < -0.5:
                regime_scores[MarketRegime.BULL_CORRECTION] = 0.7
            else:
                regime_scores[MarketRegime.BULL_CORRECTION] = 0.1
            
            # Bear Rally
            if momentum_20 < -0.02 and momentum_5 > 0.01 and bb_position > 0.5:
                regime_scores[MarketRegime.BEAR_RALLY] = 0.7
            else:
                regime_scores[MarketRegime.BEAR_RALLY] = 0.1
            
            # Breakout
            if volume_ratio > 1.5 and abs(momentum_5) > 0.03:
                regime_scores[MarketRegime.BREAKOUT] = min(1.0, volume_ratio * 0.3 + abs(momentum_5) * 10)
            else:
    ...
    async def _create_enhanced_regime_analysis(self, current_regime: MarketRegime, 
        ...
        try:
            ...
            risk_level = self._assess_regime_risk_level(current_regime, features)
            
            # Expected Volatility
            expected_volatility = self._estimate_expected_volatility(current_regime, features)
            
            # Historical Performance (mock)
            historical_performance = self._get_historical_regime_performance(current_regime)
    ...
    def _calculate_regime_duration(self, current_regime: MarketRegime) -> int:
        """
        Berechne wie lange das aktuelle Regime bereits andauert
        """
        try:
            ...
            for i in range(len(self.regime_history) - 1, 0, -1):
                if self.regime_history[i-1]['regime'] == current_regime:
                    duration += 1
                else:
                    break
            ...
            logger.error(f"❌ Regime Duration Calculation Fehler: {e}")
            return 1
    
    def _calculate_transition_probability(self, current_regime: MarketRegime) -> float:
        """
        Wahrscheinlichkeit eines Regime-Wechsels berechnen
        """
        try:
            # Basiere auf typischen Regime-Dauern
            regime_duration = self._calculate_regime_duration(current_regime)
            
            # Typical regime durations (in days)
            typical_durations = {
                MarketRegime.BULL_TRENDING: 90,
                MarketRegime.BEAR_TRENDING: 60,
                MarketRegime.SIDEWAYS_RANGE: 30,
                MarketRegime.HIGH_VOLATILITY: 10,
                MarketRegime.LOW_VOLATILITY: 45,
                MarketRegime.BULL_CORRECTION: 15,
                MarketRegime.BEAR_RALLY: 12,
                MarketRegime.BREAKOUT: 5,
                MarketRegime.ACCUMULATION: 120,
                MarketRegime.DISTRIBUTION: 90,
                MarketRegime.CRISIS: 30
            }
            
            typical_duration = typical_durations.get(current_regime, 30)
            
            # Transition probability increases with duration
            transition_prob = min(0.8, regime_duration / typical_duration)
            
            return transition_prob
            
        except Exception as e:
            logger.error(f"❌ Transition Probability Calculation Fehler: {e}")
            return 0.3
    
    def _assess_regime_risk_level(self, regime: MarketRegime, features: Dict[str, float]) -> str:
        """
        Risk Level für das aktuelle Regime bewerten
        """
        try:
            volatility = features.get('realized_volatility', 0.02)
            
            # Base risk levels by regime
            base_risk = {
                MarketRegime.BULL_TRENDING: 'medium',
                MarketRegime.BEAR_TRENDING: 'high',
                MarketRegime.SIDEWAYS_RANGE: 'low',
                MarketRegime.HIGH_VOLATILITY: 'very_high',
                MarketRegime.LOW_VOLATILITY: 'low',
                MarketRegime.BULL_CORRECTION: 'medium',
                MarketRegime.BEAR_RALLY: 'medium',
                MarketRegime.BREAKOUT: 'high',
                MarketRegime.ACCUMULATION: 'low',
            ...
            logger.error(f"❌ Risk Level Assessment Fehler: {e}")
            return 'medium'
    
    def _estimate_expected_volatility(self, regime: MarketRegime, features: Dict[str, float]) -> float:
        """
        Erwartete Volatilität für das Regime schätzen
        """
        try:
            current_vol = features.get('realized_volatility', 0.02)
            
            # Expected volatility by regime
            regime_vol_expectations = {
                MarketRegime.BULL_TRENDING: 0.02,
                MarketRegime.BEAR_TRENDING: 0.035,
                MarketRegime.SIDEWAYS_RANGE: 0.015,
                MarketRegime.HIGH_VOLATILITY: 0.06,
                MarketRegime.LOW_VOLATILITY: 0.01,
                MarketRegime.BULL_CORRECTION: 0.03,
                MarketRegime.BEAR_RALLY: 0.025,
                MarketRegime.BREAKOUT: 0.045,
                MarketRegime.ACCUMULATION: 0.012,
                MarketRegime.DISTRIBUTION: 0.022,
                MarketRegime.CRISIS: 0.08
            }
            
            expected_vol = regime_vol_expectations.get(regime, 0.025)
            
            # Blend with current volatility (70% regime expectation, 30% current)
            blended_vol = expected_vol * 0.7 + current_vol * 0.3
            
            return blended_vol
            
        except Exception as e:
            logger.error(f"❌ Expected Volatility Estimation Fehler: {e}")
            return 0.025
    
    def _get_historical_regime_performance(self, regime: MarketRegime) -> Dict[str, float]:
        """
        Historische Performance-Daten für das Regime (Mock)
        """
        try:
            ...
            return performance_data.get(regime, {
                'avg_return': 0.03,
                'volatility': 0.15,
                'sharpe_ratio': 0.5,
                'max_drawdown': 0.10,
            ...
            logger.error(f"❌ Historical Performance Fehler: {e}")
            return {'avg_return': 0.03, 'volatility': 0.15, 'sharpe_ratio': 0.5}
    ...
    async def get_regime_transition_forecast(self, days_ahead: int = 10) -> Dict[MarketRegime, float]:
        """
        Regime Transition Forecast für die nächsten N Tage
        """
        try:
            if not self.is_trained or not ADVANCED_ML_AVAILABLE:
                # Fallback forecast
                return {regime: 1.0/len(MarketRegime) for regime in MarketRegime}
            
            # Use HMM to predict future states
            current_state = list(self.regime_mappings.values()).index(self.current_regime)
            
            # Simulate future paths
            forecast_probabilities = {}
            
            for regime in MarketRegime:
                forecast_probabilities[regime] = np.random.uniform(0.05, 0.25)
            
            # Current regime has higher probability
            forecast_probabilities[self.current_regime] = 0.4
            
            # Normalize
            total_prob = sum(forecast_probabilities.values())
            forecast_probabilities = {k: v/total_prob for k, v in forecast_probabilities.items()}
            
            return forecast_probabilities
            
        except Exception as e:
            logger.error(f"❌ Regime Transition Forecast Fehler: {e}")
            return {regime: 1.0/len(MarketRegime) for regime in MarketRegime}
    
    async def get_optimal_strategies_for_regime(self, regime: MarketRegime) -> List[str]:
        """
        Optimale Strategien für ein spezifisches Regime
        """
        return self.strategy_mappings.get(regime, ['conservative'])
    
    async def get_regime_performance_forecast(self, regime: MarketRegime) -> Dict[str, float]:
        """
        Performance Forecast für ein spezifisches Regime
        """
        historical_perf = self._get_historical_regime_performance(regime)
        
        # Add some uncertainty to historical performance
        forecast = {}
        for metric, value in historical_perf.items():
            if metric in ['avg_return', 'sharpe_ratio']:
                # Add random variation
                forecast[metric] = value * (1 + np.random.normal(0, 0.1))
            else:
...
class RegimeBasedStrategySelector:
    ...
    async def select_optimal_strategy(self, market_data: Dict) -> Dict[str, Any]:
        """
        Optimale Strategie basierend auf aktuellem Regime auswählen
        """
        try:
            ...
            for strategy in optimal_strategies:
                ...
                historical_perf = regime_analysis.historical_performance.get('win_rate', 0.55)
    ...
    async def get_optimal_strategies(self, market_data: Dict) -> List[str]:
        """
        Get optimal strategies for current market regime
        """
        try:
            regime_analysis = await self.regime_detector.detect_current_regime(market_data)
            return regime_analysis.optimal_strategies
            
        except Exception as e:
            logger.error(f"❌ Get Optimal Strategies Fehler: {e}")
